...
class Scene:
    def __init__(self, cfg, obj_id, n_img, dirs, rgb, depth, frame_id) -> None:
        ...
        with nvtx_range("initialize_scene_model"):
            self.vMapModel = Model(cfg).to(self.device)
        with nvtx_range("initialize_keyframe_buffer"):
            self.kf_ids = torch.empty(
                self.keyframe_buffer_size, 1, dtype=torch.long, device=self.device
            )
            self.kf_data = torch.empty(
                self.keyframe_buffer_size, self.kf_pixels, 7, device=self.device
            )

        with nvtx_range("initial_keyframe_pixel_sampling"):
            idxs = random.sample(range(0, self.total_pixels), self.kf_pixels)
        with nvtx_range("store_initial_keyframe"):
            data = torch.cat(
                [rgb, self.dirs.rays_dir_cache, depth[..., None]], dim=-1
            ).reshape(-1, 7)
    ...
    def append_keyframe(self, rgb, depth, features, frame_id):
        ...
        if frame_id % self.keyframe_step == 0:
            with nvtx_range("keyframe_pixel_sampling"):
                if featured:
                ...
                # if Grided:
                #
            with nvtx_range("store_keyframe"):
    ...
    def get_training_samples(self, n_frames, n_samples, optimized_ids):
        with nvtx_range("build_mapping_sample_indices"):
            n_optimize_frame = len(optimized_ids)
            keyframe_ids = torch.tensor(
                optimized_ids * (n_frames // n_optimize_frame),
                dtype=torch.long,
                device=self.device,
            )
            indice = torch.randint(
                0, self.kf_pixels, (n_frames, n_samples), device=self.device
            )
        with nvtx_range("gather_mapping_samples"):
            sampled_data = torch.gather(
                self.kf_data[keyframe_ids],
                1,
                indice.unsqueeze(-1).expand(-1, -1, 7),
            )
            sampled_id = self.kf_ids[keyframe_ids]
            sampled_mask = None
            if self.cfg["do_obj"]:
                sampled_mask = torch.gather(
                    self.kf_mask[keyframe_ids],
                    1,
                    indice.unsqueeze(-1).expand(-1, -1, 1),
                )
        return sampled_data, sampled_mask, sampled_id

    def get_tracking_samples(self, n_samples, gt_color, gt_depth, features):
        with nvtx_range("build_tracking_sample_indices"):
            if not hasattr(self, 'iW'):
                self.iW = self.cfg["tracking"]["ignore_edge_W"]
                self.iH = self.cfg["tracking"]["ignore_edge_H"]
                self.tracking_pixels = (
                    self.frames_width - 2 * self.iW
                ) * (self.frames_height - 2 * self.iH)
            ...
            # if Grided:
            #
            indice = random.sample(
                range(self.tracking_pixels), int(n_samples - featureNum)
            )
            indice = torch.tensor(indice)
            indice_w, indice_h = (
                indice % (self.frames_width - self.iW * 2),
                torch.div(
                    indice,
        ...
        with nvtx_range("gather_tracking_samples"):
            sampled_rgb = gt_color[self.iW:-self.iW, self.iH:-self.iH, :][
                indice_w, indice_h
            ]
            sampled_depth = gt_depth[self.iW:-self.iW, self.iH:-self.iH][
                indice_w, indice_h
            ]
            sampled_ray_dirs = self.dirs.rays_dir_cache[
                self.iW:-self.iW, self.iH:-self.iH
            ][indice_w, indice_h]
