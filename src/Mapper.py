...
class Mapper(object):
    def __init__(self, cfg, args, slam):
        ...
        self.disable_timing_eval = bool(
            cfg.get("timing", {}).get("disable_eval", False)
        )
        self.lost = False
        self.no_inst = False
        self.pe_buffer = None
    ...
    def optimize_map(self, idx):
        with nvtx_range("choose_sliding_window"):
        ...
        # Unlike Co-SLAM, EC-SLAM samples all iteration batches up front.
        sample_range = "select_samples_FFM" if first_frame else "presample_global_rays_BA"
        with nvtx_range(sample_range):
            ...
            bg_data, bg_mask, bg_id = scene_obj.get_training_samples(
                frameNumber_bg, self.pixels_bg_per_frame, optimized_ids
            )

        setup_range = "setup_optimizer_FFM" if first_frame else "setup_optimizer_BA"
        with nvtx_range(setup_range):
            ...
            if BAFalg:
                ...
                BAPose_quad = nn.Parameter(
                    torch.cat(
                        [
                            matrix_to_quaternion(BAPose[:, :3, :3]),
                            BAPose[:, :3, 3],
                        ],
                        dim=-1,
                    )
                )
                poseOptimize = torch.optim.Adam(
                    [{'params': BAPose_quad, 'lr': 0.001}]
                )
                    ...
                    for i in optIDs:
                        quad_loss = torch.abs(
                            get_tensor_from_camera(
                                self.gt_c2w_list[i].to(self.device)
                            )
                            - get_tensor_from_camera(
                                self.estimate_c2w_list[i].to(self.device)
                            )
                        ).mean().item()
        ...
        for joint_iter in range(num_joint_iters):
            iteration_range = (
                "FFM_ITER_PROFILE_TOTAL" if first_frame else "BA_ITER_PROFILE_TOTAL"
            )
            iteration_timing = (
                nullcontext()
                if first_frame
                else self.iteration_timer.stage(
                    idx,
                    "bundle_adjustment",
                    joint_iter,
            ...
            with iteration_timing, nvtx_range(iteration_range):
                if BAFalg:
                    with nvtx_range("reconstruct_poses_BA"):
                        c2w = torch.eye(4, device=BAPose_quad.device).unsqueeze(0).repeat(
                            BAPose_quad.shape[0], 1, 1
                        )
                        c2w[:, :3, :3] = quaternion_to_matrix(BAPose_quad[:, :4])
                        c2w[:, :3, 3] = BAPose_quad[:, 4:]
                        currentEstimation = self.estimate_c2w_list.clone().to(
                            self.device
                        )
                        currentEstimation[optIDs] = c2w

                batch_range = (
                    "select_presampled_batch_FFM"
                    if first_frame
                    else "select_presampled_batch_BA"
                )
                with nvtx_range(batch_range):
                    bg_frame_idx = slice(
                        joint_iter * len(optimized_ids),
                ...
                transform_range = (
                    "ray_sampling&coords_transform_FFM"
                    if first_frame
                    else "resize_tensors_BA"
                )
                with nvtx_range(transform_range):
                    bg_c2w_idx = currentEstimation[bg_id_idx, :3, :].unsqueeze(1).expand(
                        -1, self.pixels_bg_per_frame, -1, -1
                    )
                    bg_origin_idx = bg_c2w_idx[:, :, :, 3].reshape(-1, 3)
                    bg_dirW_idx = (
                        bg_c2w_idx[:, :, :, :3] @ bg_dir_idx[..., None]
                    ).squeeze(-1).reshape(
                        [self.pixels_bg_per_frame * len(optimized_ids), 3]
                    )

                forward_range = "forward_FFM" if first_frame else "Forward_BA"
                forward_timing = (
                    nullcontext()
                    if first_frame
                    else self.iteration_timer.stage(
                ...
                with forward_timing, nvtx_range(forward_range):
                    bg_loss = scene_bg.vMapModel.forward(
                        bg_origin_idx,
                        bg_dirW_idx,
                        bg_rgb_idx,
                        bg_depth_idx,
                ...
                backward_range = "backward_FFM" if first_frame else "Backward_BA"
                backward_timing = (
                    nullcontext()
                    if first_frame
                    else self.iteration_timer.stage(
                ...
                with backward_timing, nvtx_range(backward_range):
                    bg_loss.backward()

                optimizer_range = (
                    "map_optim_FFM" if first_frame else "Optimize_map_pose_BA"
                )
                with nvtx_range(optimizer_range):
                ...
                with nvtx_range("mapping_iteration_metrics"):
                    debugShow = joint_iter % 5 == 0 and self.debug and self.showLoss
                    if (
                        debugShow or joint_iter == num_joint_iters - 1
                    ) and self.verbose:
                        print(
                            "frame_id: ",
                            self.idx[0].item(),
            ...
            with nvtx_range("update_pose_BA"):
                ...
                if self.verbose:
                    ...
                    for i in optIDs:
                        ...
                        if self.verbose:
                            print(
                                f'frame {i},camera tensor error: '
                                + f'{initLoss[index]:.4f}->{quad_loss:.4f}'
                            )
                        index += 1
        if self.do_obj:
            with nvtx_range("update_object_models_mapping"):
                with torch.no_grad():
                    model_id = 0
                    for obj_id in self.optimize_inst_ids:
                        if obj_id != 0:
                            ...
                            for i, param in enumerate(
                                scene_obj.vMapModel.fc_occ_map.parameters()
                            ):
                                param.copy_(self.fc_param[i][model_id])
                            for i, param in enumerate(
                                scene_obj.vMapModel.pe.parameters()
                            ):
    ...
    def run(self):
        ...
        while True:
            ...
            first_mapping = self.first_map
            # This parent is created inside the mapping worker thread so the
            # complete Figure 1 hierarchy is stack-nested in Nsight.
            with nvtx_range("Mapping"):
                first_mapping_scope = (
                    nvtx_range("First_frame_mapping")
                    if first_mapping
                    else nullcontext()
                )
                with first_mapping_scope:
                    if pre_mapping_index != idx:
                        with nvtx_range("frame_data_to_gpu_mapping"):
                        ...
                        with nvtx_range("add_Keyframe"):
                            if 0 in self.vis_dict.keys():
                                scene_obj = self.vis_dict[0]
                                scene_obj.append_keyframe(
                                    gt_color, gt_depth, features, idx
                                )
                            else:
                                scene_bg = Scene(
                                    self.cfg,
                        ...
                        if not hasattr(self, "train_optimiser"):
                            with nvtx_range("initialize_map_optimizer_FFM"):
                                self.train_optimiser = torch.optim.Adam(
                                    self.vis_dict[
                                        0
                                    ].vMapModel.decoder.get_optParameter(),
                                    betas=(0.9, 0.99),
                                )
                ...
                should_evaluate = (
                    (idx > 0 and idx % self.cfg["debug"]["c2wSave"] == 0)
                    or idx == self.n_img - 1
                )
                if not self.disable_timing_eval and should_evaluate:
                    with nvtx_range("trajectory_evaluation_and_io"):
                        if os.path.exists(self.ckptsdir):
                            ...
                            poses_est, _ = convert_poses(
                                estimate_c2w_list, idx, self.scale
                            )
                            ...
                            with open('{}/RMSE_rel.txt'.format(self.ckptsdir), 'a') as file:
                                file.writelines(str(results) + "\n")
                with nvtx_range("publish_mapping_state"):
